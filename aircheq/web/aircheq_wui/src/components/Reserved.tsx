...
const ReservedTitle = styled(DetailLink)`
    display: block;
`
const ReservedToStart = styled.div`
`
...
const ReservedProgram = (props: { program: model.IProgram, key: number }) => {
    ...
    const duration = program.duration.locale("ja")
    const toStart = moment.duration(start.diff(moment())).locale("ja")
    return  <ProgContainer key={key}>
                ...
                >{program.title}</ReservedTitle>
                <ReservedToStart>{toStart.humanize()}後</ReservedToStart>
...
const ReservedPrograms = styled.ul`
    padding: 0;
    margin: 0;
...
class Reserved extends React.Component <{}, ReservedState> {
    constructor() {
        super()
        this.state = {
            programs: [],
