...
interface SearchFormState extends Query {
    result: Program[]
}
...
class SearchForm extends React.Component <{}, SearchFormState> {
        ...
        return  <SearchContainer>
                    <InputForm>
                        ...
                        <Input placeholder="service" type="text"
                            ...
                            onChange={ e => { this.setState({ service: e.currentTarget.value })}}
                            onKeyPress={this.onKeyPress}
                        />
                        <Input placeholder="channel" type="text"
                            value={this.state.channel}
                            onChange={ e => { this.setState({ channel: e.currentTarget.value })}}
                            onKeyPress={this.onKeyPress}
                        />
    ...
    search() {
        ...
        const isAllEmpty = Object.keys(query).map(k => query[k]).every(v => v === "")
...
{(() => props.result.map((program: Program, i: number) => 
        <ProgContainer key={i}>
            ...
            <ProgDuration>{program.duration.locale("ja").asMinutes()}分間</ProgDuration>
        </ProgContainer>
    )
)()}
</ProgContainer>
